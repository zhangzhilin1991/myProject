...
public class RotateImageView extends ImageView {
	...
	public void setOrientation(int degree) {
		...
		mAnimationStartTime += (long) ((Math.abs(diff) * 1000) / 270);
		invalidate();
	}
		...
		if ((w != 0) && (h != 0)) {
			if (mCurrentDegree != mTargetDegree) {
				...
				if (time < mAnimationEndTime) {
					...
					// ?? (mClockwise ? deltaTime : -deltaTime * 0x10e) =
					// ((mClockwise ? deltaTime : -deltaTime * 0x10e) / 0x3e8);
					degree = degree >= 0 ? (degree % 0x168)
							: (degree % 0x168) + 0x168;
			...
			if (getScaleType() == ImageView.ScaleType.FIT_CENTER) {
				if ((width < w) || (height < h)) {
					float ratio = Math.min(((float) width / (float) w),
							((float) height / (float) h));
					canvas.scale(ratio, ratio, ((float) width / 2.0f),
							((float) height / 2.0f));
				}
			}
			canvas.translate((float) ((width / 0x2) + left),
					(float) ((height / 0x2) + top));
